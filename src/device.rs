...
use crate::{
    ...
    cstr_to_string, cstr_to_string_lossy, raw, Error,
};
...
impl Device {
    ...
    // it. The devices on either side of it are unaffected. The description is kept lossily: it is
    // only ever displayed, and on a localised Windows libpcap hands it over in the local code
    // page rather than UTF-8.
    unsafe fn from_pcap_if_t(dev: &raw::pcap_if_t) -> Option<Device> {
        Some(Device::new(
            cstr_to_string(dev.name).ok().flatten()?,
            cstr_to_string_lossy(dev.description),
